...
def create_follow_url(
    id: int, followers: bool = True, next_token=None, max_results=1000
):
    url = (
...
def is_ref_connected_to_source(
    source_id, reference_id, check_followers: bool = True, next_token=None
):
...
def decide_follower_following(json_response: dict):
    ...
    combined_followers = (
        dev1["public_metrics"]["followers_count"]
        + dev2["public_metrics"]["followers_count"]
    )
    combined_following = (
        dev1["public_metrics"]["following_count"]
        + dev2["public_metrics"]["following_count"]
    )
...
def devs_mutually_connected(dev1, dev2):
    ...
    if errors:
        return {
            "connected": False,
            "errors": [
                f"{error['value']} is not a valid user in twitter" for error in errors
            ],
        }
    ...
    if is_ref_connected_to_source(
        source_id=dev1_id, reference_id=dev2_id, check_followers=check_followers
    ) and is_ref_connected_to_source(
        source_id=dev2_id, reference_id=dev1_id, check_followers=check_followers
    ):
